...
/** Fixed deterministic seeds shared by special-weather particle renderers. */
export class SpecialWeatherParticlePool {
	...
	constructor(count: number, seed: number) {
		...
		this.drift = new Float32Array(safeCount);
		...
		for (let index = 0; index < safeCount; index += 1) {
			...
			this.scale[index] = 0.55 + unit(seed, index, 0x73) * 0.9;
			this.drift[index] = unit(seed, index, 0x64) * 2 - 1;
		}
	}
